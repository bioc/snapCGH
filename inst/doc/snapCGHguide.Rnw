...
Firstly, we use the \Rfunction{process.MAList} to 'tidy up' the MAList
...
The next step is to overcome the problem that the segmentation
functions (as written at present) can not cope with missing
values. Hence, we use an interpolating method to overcome this
problem. This is carried out using the \Rfunction{impute.lowess}
function.
