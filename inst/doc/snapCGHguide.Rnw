...
\author{ML Smith, JC Marioni, NP Thorne, T Hardcastle}
...
on the chromosome a clone if from) can be incoroporated using the \Rfunction{readPositionalInfo}
...
(e.g. if the a new version of the array output data is incompatible with the current
function) 
...
as X an Y or 23 and 24.  Both instances are handles by \Rfunction{read.clonesinfo}.
...
The following example applies the GLAD algorithm to the data, merges it
...
plotSegmentedGenome(SegInfo.Hom.merged, SegInfo.GLAD.merged, array = 1,
