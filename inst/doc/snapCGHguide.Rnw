...
\author{ML Smith, JC Marioni, NP Thorne, TJ Hardcastle}
...
If you have used the \Rpackage{snapCGH} in your work please cite the package
using the following:\\

Smith, M.L., Marioni, J.C., Thorne, N.P., Hardcastle, T.J.\\ 
...
These are respectively called in the following ways:

<<8,results=hide, eval = FALSE>>=
...
\begin{figure}[!ht]
\centering
\scalebox{1.0}[0.9]{\includegraphics{zoomGenome.pdf}}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics{zoomChromosome.pdf}
\end{figure}

\section*{Simulating Data}

\Rpackage{snapCGH} also provides facilities for simulating aCGH data.

<<label=Simulation1>>=
simulation <- simulateData(nArrays = 4)
@ 

In the following example we run the homogeneous HMM and the GLAD algorithm on 
...
The argument \Rfuncarg{offset}, which can take an integer between 0 and 2,
...
Sim.GLAD <- runGLAD(simulation)
rates <- compareSegmentations(simulation, offset = 0, Sim.HomHMM, Sim.GLAD)
@ 
...
Shown below are boxplots of the \Robejct{rates} object.
